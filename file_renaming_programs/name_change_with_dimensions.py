...
# ##IMPORTANT## #
# This program will only work with images, if other file formats
# exist in the files_to_rename folder the script will crash during the execution
...
def main():
    ...
    for count, filename in enumerate(os.listdir(folder_directory)):
        ...
        my_image = cv2.imread(source)
        width = str(my_image.shape[1])
        height = str(my_image.shape[0])

        # Final Formatting and saving of the files
        if starting_file_number + count < 10:
            destination = file_name_format + "000000" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        elif starting_file_number + count < 100:
            destination = file_name_format + "00000" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        elif starting_file_number + count < 1000:
            destination = file_name_format + "0000" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        elif starting_file_number + count < 10000:
            destination = file_name_format + "000" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        elif starting_file_number + count < 100000:
            destination = file_name_format + "00" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        elif starting_file_number + count < 1000000:
            destination = file_name_format + "0" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        elif starting_file_number + count >= 1000000:
            destination = file_name_format + "" + file_number_new + " " \
                          + width + "x" + height + str(file_extension)
            file_name = folder_directory + destination
            os.rename(source, file_name)
        else:
